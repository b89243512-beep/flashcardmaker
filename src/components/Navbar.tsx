    ...
    <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-lg border-b border-border">
      <nav className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
        ...
        <div className="hidden items-center gap-6 md:flex">
          ...
          <Link href="#maker" className="btn-primary text-sm px-4 py-2 gap-2">
            <Sparkles className="w-4 h-4" />
            Make Flashcards
          </Link>
        </div>

        <button type="button" className="md:hidden text-foreground" onClick={() => setMobileMenuOpen((p) => !p)}
      ...
      {mobileMenuOpen && (
        <div className="flex flex-col gap-1 px-4 pb-4 md:hidden bg-white border-b border-border">
          ...
          <Link href="#maker" className="btn-primary text-sm px-4 py-2.5 mt-2 gap-2" onClick={() => setMobileMenuOpen(false)}>
            <Sparkles className="w-4 h-4" />Make Flashcards
          </Link>
        </div>
      )}
    </header>
